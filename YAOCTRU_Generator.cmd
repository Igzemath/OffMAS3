...
echo. 6. Semi-Annual Enterprise              ^| Production::DC
echo.
...
if %_a64%==1 echo. 5. Windows 11    ARM64 [x64 Emulation]
echo %line%
echo.
...
set "ulp=https://c2rsetup.officeapps.live.com/c2r/download.aspx?ProductreleaseID=LanguagePack"
set oar=%arc%
...
:uPack
call :EC3HO%inpt% Setup.exe
call :EC4HO%inpt% %lang% %arc% SetupLanguagePack.%arc%.%lang%.exe
if exist "%vvv%_%arc%_%lang%_LangPack.xml" exit /b
(
echo.^<Configuration^>
echo.  ^<Add SourcePath="" OfficeClientEdition="%bit%"^>
...
echo.  ^</Add^>
echo.^</Configuration^>
)>"%vvv%_%arc%_%lang%_LangPack.xml"
exit /b

:uPackDual
call :EC4HO%inpt% %lang% x64 SetupLanguagePack.x64.%lang%.exe
if exist "%vvv%_x64_%lang%_LangPack.xml" exit /b
(
echo.^<Configuration^>
echo.  ^<Add SourcePath="" OfficeClientEdition="64"^>
echo.    ^<Product ID="LanguagePack"^>
...
:EC4HO1
(echo %ulp%^&language=%1^&platform=%2&echo.  out=%3&echo.)>>"%output%"
exit /b
...
exit /b
:EC4HO4
(echo %ulp%^&language=%1^&platform=%2)>>"%output%"
exit /b
...
:EC4HO3
(echo url %ulp%^&language=%1^&platform=%2&echo -o %destDir%\%3)>>"%output%"
exit /b
...
echo if exist "%%uri%%" del /f /q "%%uri%%"
if %full%==0 (
echo if exist "%vvv%_x86_%lang%_LangPack.xml" move /y "%vvv%_x86_%lang%_LangPack.xml" "%%destDir%%\Office\LangPack_%lang%_x86.xml"
echo if exist "%vvv%_x64_%lang%_LangPack.xml" move /y "%vvv%_x64_%lang%_LangPack.xml" "%%destDir%%\Office\LangPack_%lang%_x64.xml"
)
if %proof%==1 (
...
if %full%==0 (
echo if exist "%vvv%_x86_%lang%_LangPack.xml" move /y "%vvv%_x86_%lang%_LangPack.xml" "%%_rot%%\Office\LangPack_%lang%_x86.xml"
echo if exist "%vvv%_x64_%lang%_LangPack.xml" move /y "%vvv%_x64_%lang%_LangPack.xml" "%%_rot%%\Office\LangPack_%lang%_x64.xml"
)
if %proof%==1 (
